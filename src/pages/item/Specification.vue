  ...
  export default {
    ...
    methods: {
      ...
      deleteGroup(i) {
          ...
          this.$http.get("/item/spec/delete",{params:{"category_id":this.currentNode.id}}).then(resp=>{
              this.$message.error("删除成功!");
              this.getDataFromServer();
          }).catch(()=>{
              this.$message.error("删处失败!");
          });
      ...
      deleteParam(i, j) {
        ...
        this.specifications[i].params.splice(j, 1);
          console.log(this.specifications.splice(i, 1))
      },
      // 删除指定值
      deleteOption(i1, i2, i3) {
          console.log("333333")
       this.specification=this.specifications[i1].params[i2].options.splice(i3, 1);
